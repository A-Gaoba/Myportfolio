    ...
    <div className='flex w-full justify-between  -mt-2 z-10 ml-2'>
      ...
      <div className={'flex justify-evenly md:border-2 md:p-5 p-2 rounded-bl-2xl rounded-tr-lg ' + (isNavOpen ? "w-full mr-2" : "w-2/3 2xl:w-1/2")}>
        ...
        <div className="md:hidden z-10" onClick={toggleNav}>
          {isNavOpen ? <FaTimes size={40} className="text-gray-500 absolute top-22 right-6 border-2 p-1 rounded-lg" /> : <FaBars size={40} className="text-gray-500 absolute top-22 right-6 border-2 p-1 rounded-lg" />}
        </div>
        {isNavOpen && (
